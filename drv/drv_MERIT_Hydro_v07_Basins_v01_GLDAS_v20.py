...
# *****************************************************************************
# Purpose:
# This program is designed to produce namelist file for RAPID for one
# entire month, and can be run with a simple set of instructions.
# Example usage:
#   rapid = RAPID('74', 'VIC', '3H', '2000-01')    # to create the RAPID class,
#                                                     paths and variabless
#   drv_write_namelist(rapid)                      # to write the namelist file
#   drv_run(rapid)                                 # to run RAPID
# This class combines the pfaf_level_02 code, Land Surface Model (LSM) name,
# the LSM temporal resolution, and month in yyyy-mm format to create namelist
# Authors:
# Cedric H. David, Quentin Bonassies, Manu Tom, 2024-2024
...
class RAPID(object):
    ...
    def set_paths(self):
        # Uses “formatted string literals”,
        # also known as f-strings.
        # ---------------------------------------------------------------------
        # Directories
        # ---------------------------------------------------------------------
        self.inp_dir = f'/rapid/input/pfaf_{self.basn_id}/'
        self.out_dir = '/tmp/'
        ...
        # ---------------------------------------------------------------------
        self.namelst = (
            f'rapid_namelist_pfaf_{self.basn_id}_GLDAS_{self.lsm_mod}'
            f'_{self.lsm_stp}_{self.yyyy_mm}'
            )
        # ---------------------------------------------------------------------
        # Hydrography {basn_id}
        # ---------------------------------------------------------------------
        ...
        self.qini_nc = (
            f'Qinit_pfaf_{self.basn_id}_GLDAS_{self.lsm_mod}_'
            f'{self.lsm_stp}_{self.yyyy_mm}_utc.nc'
            )
        self.qfin_nc = (
            f'Qfinal_pfaf_{self.basn_id}_GLDAS_{self.lsm_mod}_'
            f'{self.lsm_stp}_{self.yyyy_mm}_utc.nc'
            )

        self.qout_nc = (
            f'Qout_pfaf_{self.basn_id}_GLDAS_{self.lsm_mod}_'
            f'{self.lsm_stp}_{self.yyyy_mm}_utc.nc'
            )
        self.vfil_nc = (
            f'V_pfaf_{self.basn_id}_GLDAS_{self.lsm_mod}_'
            f'{self.lsm_stp}_{self.yyyy_mm}_utc.nc'
            )
        ...
        # ---------------------------------------------------------------------
        self.m3r_ncf = (
            f'm3_riv_pfaf_{self.basn_id}_GLDAS_{self.lsm_mod}_'
            f'{self.lsm_stp}_{self.yyyy_mm}_utc.nc4'
            )
    ...
    def set_vars(self):
        ...
        # Temporal variables
        # ---------------------------------------------------------------------
        self.zs_Taum = datetime.timedelta(
            seconds=(self.dat_end - self.dat_str).total_seconds() + 1
            ).total_seconds()
...
def drv_write_namelist(rapid: RAPID):
    ...
    sep = "!" + "*"*79 + '\n'
    empty = "" + '\n'
    with open(rapid.out_dir + rapid.namelst, "w") as f:
        f.write("&NL_namelist" + "\n")
        f.write("!" + "*"*79 + "\n")
        f.write(f"!{rapid.out_dir + rapid.namelst}\n")
        f.write("!" + "*"*79 + "\n")
        f.write(empty)
        f.write("!Purpose:" + "\n")
        f.write(
            "!    rapid_namelist file created automatically with the RAPID "
            "class\n"
            )
        ...
        if rapid.yyyy_mm == '1979-12':
            f.write("BS_opt_Qinit = " + FALSE + "\n")
        else:
            f.write("BS_opt_Qinit = " + TRUE + "\n")

        f.write("BS_opt_Qfinal = " + TRUE + "\n")
        f.write("BS_opt_V = " + FALSE + "\n")
        ...
        f.write(
            "rapid_connect_file = '" + rapid.inp_dir + rapid.con_csv + "'\n"
            )
        f.write("Vlat_file = '" + rapid.out_dir + rapid.m3r_ncf + "'\n")
        f.write(empty)
...
def drv_run(rapid: RAPID):
    ...
    path_nlst = rapid.out_dir + rapid.namelst
    print("Run RAPID model...")
    subprocess.run(["mpiexec", "-n", "1", "/home/rapid/tst/rapid", "-nl",
                    path_nlst, "-ksp_type", "preonly"], capture_output=False,
